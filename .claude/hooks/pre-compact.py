...
def find_active_plan(project_dir: str) -> dict | None:
    ...
    plans_dir = Path(project_dir) / "01_project_planning" / "quality_reports" / "plans"
...
def extract_recent_decisions(project_dir: str, limit: int = 3) -> list[str]:
    ...
    logs_dir = Path(project_dir) / "01_project_planning" / "quality_reports" / "session_logs"
...
def append_to_session_log(project_dir: str, trigger: str) -> None:
    ...
    try:
        with open(log_files[0], "a") as f:
            ...
            f.write(f"Check git log and 01_project_planning/quality_reports/plans/ for current state.\n")
    except IOError:
        pass
